...
public class ExportExcelService : IExportExcelService
{
    public async Task<byte[]?> GenerateExcel(List<CaseEntity> cases)
    {
        try
        {
            ...
            foreach (var c in cases)
            {
                ...
                worksheet.Cell(string.Format("G{0}", i)).Value =
                    !string.IsNullOrEmpty(c.OriginDocument?.Description)
                        ? c.OriginDocument.Description
                        : string.Empty;
                worksheet.Cell(string.Format("H{0}", i)).Value = !string.IsNullOrEmpty(c.Brand?.Description)
                    ? c.Brand.Description
                    : string.Empty;
                ...
                worksheet.Cell(string.Format("L{0}", i)).Value = !string.IsNullOrEmpty(c.Department?.Description)
                ...
                worksheet.Cell(string.Format("Q{0}", i)).Value = !string.IsNullOrEmpty(c.Province?.Description)
                    ? c.Province.Description
                    : string.Empty;
                ...
                worksheet.Cell(string.Format("Y{0}", i)).Value = !string.IsNullOrEmpty(c.CaseStatus?.Description)
                    ? c.CaseStatus.Description
                    : string.Empty;
                ...
                worksheet.Cell(string.Format("AC{0}", i)).Value =
                    c.ObservationDepartment + " / " + c.ObservationExtension;
            }
    ...
    public Task<byte[]?> GenerateExcelV1(List<CaseEntity> cases)
    {
        var dt = new DataTable();
        dt.Columns.AddRange(new DataColumn[]
        {
            new("Fecha Recepción"),
            new("Recibido Fisicamente"),
            new("Nro Circular"),
            new("Fecha Circular"),
            new("Nro Referido SBS"),
            new("Fecha Emisión"),
            new("Origen"),
            new("Dirigido a"),
            new("Descripcion"),
            new("Prioridad"),
            new("Contesta Secretaria"),
            new("Area Responde"),
            new("Funcionario Asignado"),
            new("Requiere mas Informacion"),
            new("Area Funcionario Mas Informacion"),
            new("Fecha Traslado"),
            new("Lugar Destino"),
            new("Fecha Vencimiento"),
            new("Plazo Respuesta"),
            new("Nro Oficio Respuesta"),
            new("Fecha Contestacion"),
            new("Demora Responder"),
            new("Funcionario Responde"),
            new("Fecha Respuesta"),
            new("Estado Caso"),
            new("Fecha Acuse"),
            new("Demora Dias"),
        ...
        foreach (var c in cases)
        {
            dt.Rows.Add(
                ...
                c.SbsNumber,
                c.IssueDate,
                c.OriginDocument?.Description ?? string.Empty,
                c.Brand?.Description ?? string.Empty,
                c.Description,
                "Normal",
                string.Empty,
                c.Department?.Description ?? string.Empty,
                c.User?.FullName,
                string.Empty,
                "N/A",
                c.TransferDate,
                c.Province?.Description ?? string.Empty,
                c.Deadline,
                c.Deadline - c.ReceptionDate,
                c.DocumentNumber,
                c.ResponseDate,
                c.ResponseDate - c.ReceptionDate,
                c.User?.FullName,
                c.ReplyDate,
                c.CaseStatus?.Description ?? string.Empty,
                c.AcknowledgmentDate,
                c.AcknowledgmentDate - c.ReceptionDate,
                c.JudgmentNumber,
                c.ObservationDepartment + " / " + c.ObservationExtension
            );
        }
        ...
        const string bookName = "Casos Secretaria";

        book.Workbook.Worksheets.Add(bookName);
        var ws = book.Workbook.Worksheets[bookName];

        ws.Cells["A3"].LoadFromDataTable(dt, true);

        ws.Cells["A3:AC3"].Style.Font.Bold = true;
        ws.Cells["A3:AC3"].Style.Fill.PatternType = ExcelFillStyle.Solid;
        ws.Cells["A3:AC3"].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(0, 112, 192));
        ws.Cells["A3:AC3"].Style.Font.Color.SetColor(Color.White);
        ws.Cells["A3:AC3"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
        ws.Cells["A3:AC3"].Style.VerticalAlignment = ExcelVerticalAlignment.Center;
        ws.Cells["A3:AC3"].Style.Border.BorderAround(ExcelBorderStyle.Thin, Color.Black);

        ws.Columns[3, dt.Columns.Count].AutoFit();

        // Return a byte array of the Excel file
        return book.GetAsByteArrayAsync();
    }
}
