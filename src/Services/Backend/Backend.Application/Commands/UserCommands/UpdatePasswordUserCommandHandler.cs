    ...
    : IRequestHandler<UpdatePasswordUserCommand, EntityResponse<bool>>
{
    ...
    private async Task UpdatePasswordUser(UpdatePasswordUserCommand command, User applicationUser, CancellationToken cancellationToken)
    {
        applicationUser.Password = command.NewPassword;
