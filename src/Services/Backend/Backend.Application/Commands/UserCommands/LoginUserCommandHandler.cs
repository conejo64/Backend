...
public class LoginUserCommandHandler : IRequestHandler<LoginUserCommand, EntityResponse<ReadUserMeResponse>>
{
    ...
    private async Task<ICollection<string>> GetPermissions(User applicationUser, CancellationToken cancellationToken)
    {
        var scopedPermissions = await GetScopedPermissions(applicationUser, cancellationToken);
        var globalPermissions = await GetGlobalPermissions(applicationUser.Id, cancellationToken);
    ...
    private async Task<ICollection<string>> GetScopedPermissions(User applicationUser, CancellationToken cancellationToken)
    {
